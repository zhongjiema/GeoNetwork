...
const (
	...
	ES_URL = "http://35.236.0.52:9200"
	//ES_URL      = "http://104.198.51.243:9200"
	BUCKET_NAME = "post-image-around"

	ENABLE_BIGTABLE = false

	BIGTABLE_PROJECT_ID = "around-227207"
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	// ML Engine only supports jpeg. HW: ADD a trasfer method for other image format
	if suffix == ".jpeg" {
...
func saveToGCS(r io.Reader, bucketName, objectName string) (*storage.ObjectAttrs, error) {
	...
	client, err := storage.NewClient(ctx, option.WithCredentialsFile("Around-cfb0267755d3.json"))
	if err != nil {
		return nil, err
	}
...
func saveToBigTable(p *Post, id string) {
	ctx := context.Background()
	bt_client, err := bigtable.NewClient(ctx, BIGTABLE_PROJECT_ID, BT_INSTANCE, option.WithCredentialsFile("Around-cfb0267755d3.json"))
	if err != nil {
		panic(err)
		return
	}
